...
int hostapd_get_plan_info(char *plan, u8 plan_id)
{
	...
	if (plan_length == 0)
		wpa_printf(MSG_ERROR, "No valid plan info for plan id %u\n",plan_id);
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
		...
		bss->multi_ap_backhaul_ssid.wpa_passphrase = os_strdup(plain);
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
		...
		bss->eap_skip_prot_success = atoi(pos);
			...
			if (plain) {
				wpa_hexdump_ascii_key(MSG_MSGDUMP, "Decrypted PSK", (u8 *) plain, strlen(plain));
				if (parse_sae_password(bss, plain) < 0) {
