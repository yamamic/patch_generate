	       ...
	       u8 *mic)
{
	...
	if (mlie) {
		if (ml_parse_ie(mlie, mlie_len, &ml_info, NULL) == 0) {
			...
			wpa_printf(MSG_INFO, "FT: ml_info.link_id = %u",
					ml_info.link_id);

			if (!ml_info.link_id) {
				wpa_printf(MSG_INFO, "FT: setup_link_id = %u",
					setup_link_id);
			...
			if (!tx_reassoc_rsp) {
				for (i = 0; i < ml_info.prof_num; i++) {
					...
					wpa_printf(MSG_INFO, "FT: link_id = %u",
						link_id);
				...
				wpa_printf(MSG_INFO, "FT: MLO valid links=%x, need_ml_rsn=%u",
				...
				for (i = 0; i < ml_group->ml_link_num; i++) {
					...
					if (!os_memcmp(link->addr, main_link_addr, ETH_ALEN)) {
						...
						wpa_printf(MSG_DEBUG, "FT: tx_reassoc_rsp ml_info.link_id = %u",
							ml_info.link_id );
					}
				}
				...
				wpa_printf(MSG_INFO, "FT: MLO valid links=%x, need_ml_rsn=%u for tx_reassoc_rsp",
					valid_links, need_ml_rsn);
				wpa_printf(MSG_INFO, "FT: ml_group->ml_link_num = %zu",
...
static int wpa_parse_generic(const u8 *pos, struct wpa_eapol_ie_parse *ie)
{
	...
	if (left >= RSN_MLO_GTK_KDE_PREFIX_LENGTH &&
		...
		if (link_id >= MAX_NUM_MLO_LINKS) {
			wpa_printf(MSG_ERROR, "MLO_GTK link_id(%u) is invalid\n", link_id);
			return 2;
		}
	...
	if (left >= RSN_MLO_IGTK_KDE_PREFIX_LENGTH &&
		...
		if (link_id >= MAX_NUM_MLO_LINKS){
			wpa_printf(MSG_ERROR, "MLO_IGTK link_id(%u) is invalid\n", link_id);
			return 2;
		}
	...
	if (left >= RSN_MLO_BIGTK_KDE_PREFIX_LENGTH &&
		...
		if (link_id >= MAX_NUM_MLO_LINKS){
			wpa_printf(MSG_ERROR, "MLO_BIGTK link_id(%u) is invalid\n", link_id);
			return 2;
		}
	...
	if (left >= RSN_MLO_LINK_KDE_FIXED_LENGTH &&
		...
		if (link_id >= MAX_NUM_MLO_LINKS){
			wpa_printf(MSG_ERROR, "MLO_LINKS link_id(%u) is invalid\n", link_id);
			return 2;
		}
