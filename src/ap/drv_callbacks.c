		...
		if (hapd->disabled && hapd->started) {
			...
			if (hapd->wpa_auth) {
				if (hapd->wpa_auth->group) {
					if (hapd->wpa_auth->group->wpa_group_state == WPA_GROUP_FATAL_FAILURE) {
						wpa_printf(MSG_DEBUG, "wpa_group_state is WPA_GROUP_FATAL_FAILURE, recover to WPA_GROUP_SETKEYSDONE");
						hapd->wpa_auth->group->wpa_group_state = WPA_GROUP_SETKEYSDONE;
					}
				}
