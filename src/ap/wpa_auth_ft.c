		...
		u8 *buf, size_t len, bool is_auth)
{
	...
	*pos++ = 7;
	...
	if (!is_auth && hapd && hapd->ml_group && hapd->ml_group->links) {
		...
		wpa_printf(MSG_DEBUG, "hapd->ml_group->links->link_id=%u", hapd->ml_group->links->link_id);
		...
		for (i = 0; i< ml_link_num; i++) {
			...
			*/
			if (i == hapd->ml_group->links->link_id) {
				wpa_printf(MSG_DEBUG, "skip per-sta profile for own link(link_id=%u)", i);
				continue;
			}
	...
	)
{
	...
	wpa_hexdump(MSG_DEBUG, "FT: ftie",
						ftie, ftie_len);
	wpa_printf(MSG_INFO, "FT: wpa_ft_mic --> key_len = %zu, sm->wpa_key_mgmt = %d",
	...
	else if (auth_alg == WLAN_AUTH_FT && !sm->dot11MultiLinkActivated)
	...
	wpa_printf(MSG_DEBUG, "11 elem_count = %u", *elem_count );
	...
	*resp_ies_len = 0;
	...
	if (parse.mlie && parse.mlie_len > 0) {
		...
		if (ml_parse_ie(ml_ie, ml_ie_len, &ml, NULL) != 0) {
			...
			wpa_printf(MSG_INFO, "FT: ML: new STA:dot11MultiLinkActivated(%u),(ml_addr:" MACSTR ")",
