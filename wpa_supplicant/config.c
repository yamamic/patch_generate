...
#ifdef ENABLE_VALENC
#include <tzproc/value_encrypt.h>
#endif
				...
				int line, const char *value)
{
	...
	if(data != NULL && os_strcmp(data->name, "sae_password") == 0)
	{
		#ifdef ENABLE_VALENC
			if (*value == '"')
			{
				...
				if(pos == NULL || pos[1] != '\0')
				{
					wpa_printf(MSG_ERROR, "Line %d: strrchr failed %s ", line, data->name);	
					return -1;	
				}
				res_len = pos - value;
				plain = dup_binstr(value, res_len);
				if (plain == NULL)
				{
				...
				tmp = valenc_decrypt(plain);
				str_clear_free(plain);
				if(tmp)
				{
					wpa_hexdump_ascii_key(MSG_MSGDUMP, "Decrypted PSK", (u8 *) tmp, strlen(tmp));
				}
				else
				{
					wpa_printf(MSG_ERROR, "Failed to decrypt PSK");
					return -1;
				}
			}
		#else
				...
				const char *value)
{
	...
	if (*value == '"') {
		...
		dup_value = dup_binstr(value, len);
		if (!dup_value) {
			return -1;
		}

#ifdef ENABLE_VALENC
		plain = valenc_decrypt(dup_value);
		str_clear_free(dup_value);
		if (plain) {
			wpa_hexdump_ascii_key(MSG_MSGDUMP, "Decrypted PSK", (u8 *) plain, strlen(plain));
		} else {
			wpa_printf(MSG_ERROR, "Failed to decrypt PSK");
			return -1;
		}
		...
		if (len < 8 || len > 63) {
			wpa_printf(MSG_ERROR, "Line %d: Invalid passphrase "
				   ...
				   line, (unsigned long) len, plain);
			str_clear_free(plain);
			return -1;
		}
		wpa_hexdump_ascii_key(MSG_MSGDUMP, "PSK (ASCII passphrase)",
				      (u8 *) plain, len);
		if (has_ctrl_char((u8 *) plain, len)) {
			wpa_printf(MSG_ERROR,
		...
		if (ssid->passphrase && os_strlen(ssid->passphrase) == len &&
		    os_memcmp(ssid->passphrase, plain, len) == 0) {
			/* No change to the previously configured value */
			str_clear_free(plain);
			return 1;
		}
