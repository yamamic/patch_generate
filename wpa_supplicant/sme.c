						 ...
						 bool *ret_use_pk)
{
	...
	if (wpa_s->sme.sae.dot11MultiLinkActivated) {
		...
		wpa_printf(MSG_DEBUG, "(%s)[%d]ML: UPDATE dot11MultiLinkActivated(%u), own_ml_addr:"MACSTR"",
		...
		if (bss->dot11MultiLinkActivated) {
			...
			wpa_printf(MSG_DEBUG, "(%s)[%d] ML:SAE dot11MultiLinkActivated: %u(%u), own_addr:"MACSTR" peer addr:""MAC"MACSTR"\n",
